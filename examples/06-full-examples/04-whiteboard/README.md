...
cursors with names, avatars of who is here, undo and redo, a QR code to join from a phone, export to PNG, light and
dark.
...
| Keyboard                   | `onKey` flows on the board: a key per tool, ⌘Z/⌘⇧Z, ⌘D, ⌘A, `[`/`]`, `+`/`−`, `F`, ⌘0, Delete, `?`, Escape                                                    |
