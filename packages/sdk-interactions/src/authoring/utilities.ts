...
// Built-in `utility`-type actions, beside the implementations they describe (`../utility`). A utility is NOT
// provided by any element or source module — the runtime resolves it as `utility[action]`, so its node
// `elementId` is irrelevant. This catalog is the authoritative list of valid utility actions and their EXACT param
// names — the class of mistake it guards against is an invented param (e.g. `delay` on delayTime instead of
// `time`, which leaves the wait at 0ms).

export type BuiltinUtility = BuiltinActionSpec;
