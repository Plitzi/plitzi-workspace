...
(`node_modules/@plitzi/sdk-authoring/dist/index.d.ts`) — read it instead of guessing a name or a param.
...
2. **Change the declaration, then author it.** `npm run author` (or restart the server) runs `authorSpace`. A refusal
   is an error that names the element and the fix — fix the declaration, never work around the check.
3. **Read every warning.** Each one names something written that will not do what it says. Zero warnings is the bar.
4. **Look at it.** `npm run visual` opens the page in a browser. Check desktop, tablet and mobile, light and dark,
   and the page while its data is still loading — not only the finished frame.
...
5. **Know where a template is evaluated.** A binding's `twigTemplate` and a step's params evaluate full Twig. An
   ATTRIBUTE only resolves a name with filters (`{{ post.slug|url_encode }}`); a condition there is used as written.
   Inside any template a source is spelled in full — `apiContainer_stats`, `list_rows` — never by the element's short
   id. See [templates](reference/templates.md).
...
| [data-and-visibility.md](reference/data-and-visibility.md) | Bindings, providers, loading/empty/error states, live data, caching, showing and hiding |
