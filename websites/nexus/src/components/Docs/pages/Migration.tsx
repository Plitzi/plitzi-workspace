...
setCount(n => n + 1);`}
    ...
    <p>
      Atom/proxy stores give you granular reads for free. @plitzi/nexus gets the same granularity from{' '}
    ...
    <ul>
      ...
      <li>
        Valtio&apos;s auto-tracking is implicit; @plitzi/nexus is explicit: you declare which paths a component depends on.
    ...
    <p>
      ...
      computed values. @plitzi/nexus uses an immutable tree with explicit path subscriptions — the same granularity, but
...
const increment = () => store.setState('count', n => n + 1);`}
    ...
    <p>
      ...
      every read in <code>useMemo</code>. @plitzi/nexus eliminates the problem entirely — a component subscribes to exactly
      the path it needs.
    </p>
    <CodeBlock
