...
import type { PathOf } from '@plitzi/sdk-store';
...
const CartItemRow = ({ id, name, price, qty }: CartItemRowProps) => {
  ...
  return (
    <div className="flex items-center gap-3 border-b border-ink-800 py-2 last:border-0">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="flex h-7 w-7 items-center justify-center rounded-md border border-ink-600 bg-ink-800 text-zinc-300 transition hover:border-brand-500 hover:text-white"
        >
          −
        </button>
      ...
      <div className="w-16 text-right font-mono text-sm text-brand-300">${price * qty}</div>
    </div>
  );
};
