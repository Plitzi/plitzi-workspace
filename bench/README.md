...
Load and footprint benchmarks for the self-hosted servers. Each target is a real server — the examples, as their
READMEs start them, and `sdk-server` on its own — held to a hardware profile and driven at several concurrencies.
What it reports is what sizing hardware needs: requests a second, latency, CPU per request, and memory at rest, under
load and after it.
...
limits; nothing is installed, so run `yarn build:dev` first — the servers load the packages' `dist`. The first run
downloads the Linux build of the esbuild the workspace uses (tsx and the plugin compiler drive it) into `.cache/`.
...
`targets.ts` lists them. Each names its working directory, entry point, the path that proves it started, and its
scenarios. `runner` is how the entry is started: `tsx` as the examples document it, or `node` — Node's own type
stripping, with no transpiler loaded beside the server. `--runner` overrides it for every target.

A target that needs something the bench does not start (a database, the cloud) says so in `requires` and is left out
of `all`; name it to run it when that something is there.
