...
export const renderTool = defineTool({
  ...
  description:
    ...
    'order (heading over paragraph, forms, steps, prose). Nothing carries a minimum size, so an element with no ' +
    'content and no size takes none — give a spacer or a rail its own height/width — while heading/paragraph do ' +
    'keep the margins the browser gives them (zero them, space with the parent gap).\n' +
