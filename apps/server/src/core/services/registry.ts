...
import { widgetProxyStage } from './proxy';
...
export const buildPagePipeline = (services: ResolvedServices): Stage<SSRContext>[] => {
  ...
  if (services.mcp) {
    // Before the MCP endpoint itself: both are public and self-gated, and a widget's resources must stay
    // reachable without the auth middleware chain below.
    stages.push(widgetProxyStage, mcpStage);
  }
