...
describe('listPluginsTool', () => {
  ...
  describe('outputSchema', () => {
    it('validates a list of plugins', () => {
      expect(
        listPluginsTool.mcpDefinition.outputSchema?.safeParse([
          { name: '@plitzi/plitzi-sdk', version: '1.0.0', description: 'Core SDK' }
        ]).success
      ).toBe(true);
    });

    it('validates a plugin without optional fields', () => {
      expect(listPluginsTool.mcpDefinition.outputSchema?.safeParse([{ name: 'my-plugin' }]).success).toBe(true);
    });

    it('validates an empty plugin list', () => {
      expect(listPluginsTool.mcpDefinition.outputSchema?.safeParse([]).success).toBe(true);
    });
    ...
    it('rejects plugin without name', () => {
      expect(listPluginsTool.mcpDefinition.outputSchema?.safeParse([{ version: '1.0.0' }]).success).toBe(false);
    });
  });
});
