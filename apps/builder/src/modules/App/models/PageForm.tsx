...
const pageFormSchema = z.object({
  ...
  id: z.string().refine(isValidElementId, { message: 'Start with a letter, then letters, numbers and hyphens' }),
...
const PageForm = ({
  ...
  const form = useForm({ defaultValues: { name, slug, id, pageFolder }, config: { schema: pageFormSchema } });
  ...
  const handleChangeName = useCallback(
    (value: string) => {
      ...
      form.formMethods.setValue('id', derived);
    },
    [form.formMethods]
  );
  ...
  return (
    <Form form={form} onSubmit={handleSubmitInternal} className="gap-4">
      <Form.Body>
        ...
        <Form.Input name="id" label="Name" size="sm" />
