...
const SectionLabel = ({ children }: { children: React.ReactNode }) => (
  <div className="mb-1.5 font-mono text-[10px] uppercase tracking-widest text-zinc-400 dark:text-zinc-600">
    {children}
  </div>
);

const ColorScaleRow = ({
  label, scale, onCopy, copied
}: {
  label: string; scale: ColorScale; onCopy: (hex: string) => void; copied: string | null;
}) => {
  const shades = sortedShades(scale);
  return (
    <div className="mb-2">
      <div className="mb-0.5 font-mono text-[10px] capitalize text-zinc-500">{label}</div>
  ...
  <div className="mb-3">
    ...
    <div className="rounded border border-zinc-100 bg-white p-2 dark:border-zinc-800 dark:bg-zinc-950">
      <div className="mb-1.5 flex gap-3 text-zinc-600 dark:text-zinc-400">
        <span><strong className="text-zinc-800 dark:text-zinc-200">Heading</strong> — {typography.fontFamily.heading}</span>
        <span><strong className="text-zinc-800 dark:text-zinc-200">Body</strong> — {typography.fontFamily.body}</span>
      </div>
      {typography.scale && (
...
const buildCssVars = (
  ...
  if (colors.semantic) {
    ...
    if (success) vars.push({ varName: '--color-success', value: success, preview: success });
    if (warning) vars.push({ varName: '--color-warning', value: warning, preview: warning });
    if (error) vars.push({ varName: '--color-error', value: error, preview: error });
    if (info) vars.push({ varName: '--color-info', value: info, preview: info });
  }
  if (typography) {
...
const AIStyleGuidePreview = ({
  name, description, colors, colorsDark, typography, spacing, borderRadius, shadows, mode
  ...
  const handleConfirm = () => {
    ...
    if (borderRadius) parts.push(`Border radius:\n${borderRadius.map(t => `• ${t.name}: ${t.value}`).join('\n')}`);
    onSendMessage(
      ...
      <div className="bg-zinc-50 dark:bg-zinc-950">
        ...
        <div className="border-b border-zinc-100 dark:border-zinc-800">
          <button
            ...
            className="flex w-full items-center justify-between px-3 py-1.5 text-left font-mono text-[10px] uppercase tracking-widest text-zinc-500 hover:bg-zinc-100 dark:text-zinc-500 dark:hover:bg-zinc-900"
          >
            Colors
        ...
        {hasTokens && (
          <div>
            ...
            {open.tokens && (
              <div className="px-3 pb-2 pt-1">
                {spacing && (<><SectionLabel>Spacing</SectionLabel><TokenRow tokens={spacing} label="spacing" /></>)}
                {borderRadius && (<><SectionLabel>Border Radius</SectionLabel><RadiusRow tokens={borderRadius} /></>)}
                {shadows && (<><SectionLabel>Shadows</SectionLabel><ShadowRow tokens={shadows} /></>)}
              </div>
            )}
          </div>
        )}
