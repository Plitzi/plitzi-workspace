import getEdgeCenter from './getEdgeCenter';
...
/**
 * The `getSmoothStepPath` util returns everything you need to render a stepped path between two nodes. The
 * `borderRadius` property can be used to choose how rounded the corners of those steps are.
 *
 * @remarks
 ...
 *   ```;
 *
 * @returns A path string you can use in an SVG, the `labelX` and `labelY` position (center of path) and `offsetX`,
 *   `offsetY` between source handle and label.
 *
