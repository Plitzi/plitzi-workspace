# Documentación de Plitzi (español)

Guías para trabajar con el monorepo `plitzi-workspace`.

| Guía | Descripción |
|------|-------------|
| [Onboarding](./onboarding.md) | **Empieza aquí.** El mapa: qué existe, cómo encajan los repositorios y cómo levantar el stack completo |
| [Primeros pasos](./getting-started.md) | Clonar, instalar dependencias y ejecutar las apps |
| [Configuración local](./local-setup.md) | Servidores dev para contribuidores; hosts/HTTPS local opcional para maintainers |
| [Estructura del repositorio](./repository-structure.md) | Organización de apps y paquetes |
| [Desarrollo](./development.md) | Stack, comandos y flujo de contribución |
| [Pruebas](./testing.md) | Vitest para unidades, Playwright para lo que abre un navegador |
| [Publicaciones](./releases.md) | Versionado y publicación con Changesets |
| [Conectores y elementos de contenido](./connectors.md) | Cómo conectar un CMS externo y publicar páginas índice/detalle |

## Documentación por paquete

Algunos paquetes mantienen su propio README junto al código (en inglés):

| Paquete | Ruta |
|---------|------|
| Servidor SSR | [apps/server/README.md](../../apps/server/README.md) |
| Store (Nexus) | Repositorio propio: [Plitzi/nexus](https://github.com/Plitzi/nexus) |

## Otros idiomas

- [Documentation in English](../en/README.md)

## Archivos relacionados (raíz del repositorio)

| Español | English |
|---------|---------|
| [README.es.md](../../README.es.md) | [README.md](../../README.md) |
| — | [claude.md](../../claude.md) |
| — | [CODE_OF_CONDUCT.md](../../CODE_OF_CONDUCT.md) |
| [CONTRIBUTOR_TOS.es.md](../../CONTRIBUTOR_TOS.es.md) | [CONTRIBUTOR_TOS.md](../../CONTRIBUTOR_TOS.md) |
| [COMMERCIAL_LICENSE.es.md](../../COMMERCIAL_LICENSE.es.md) | [COMMERCIAL_LICENSE.md](../../COMMERCIAL_LICENSE.md) |
