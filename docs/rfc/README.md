...
Each RFC is a numbered, self-contained document. `Proposal` means it is recorded
for discussion and is **not implemented** yet.
...
| [0008](./0008-data-providers-and-collections-removal.md) | Data providers, RSC connectors, and Collections removal | Proposal | `sdk-shared`, `sdk-elements`, `sdk-interactions`, builder, `apps/server`, `plitzi-sdk-server` |

## Conventions

- **Filename:** `NNNN-kebab-case-title.md` (zero-padded sequential number).
- **Header:** Status, Author, Date, Scope.
- **Status values:** `Proposal` → `Accepted` → `Implemented` (or `Rejected` / `Superseded`).
