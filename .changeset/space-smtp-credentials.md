...
- **Offered by every server.** There is no deployment transport any more: `ActionEmailAdapter` is gone, and
  `action.email` is now `ActionEmailConfig` — `dailyLimitPerSpace` (default 200, counted in `kv`) and
  `allowPrivateHosts` (default off: a credential may not name `localhost` or a private network).
